...
<xsl:stylesheet version="2.0"
  ...
  <!-- A set of templates use to convert thesaurus concept to
       iso19115-3.mgb fragments. -->

  <!-- Convert a concept to an iso19115-3.mgb fragment with an Anchor
        for each keywords pointing to the concept URI-->
  <xsl:template name="to-iso19115-3.mgb-keyword-with-anchor">
    <xsl:call-template name="to-iso19115-3.mgb-keyword">
      <xsl:with-param name="withAnchor" select="true()"/>
    </xsl:call-template>
  </xsl:template>


  <!-- Convert a concept to an iso19115-3.mgb mri:MD_Keywords with an
       XLink which will be resolved by XLink resolver. -->
  <xsl:template name="to-iso19115-3.mgb-keyword-as-xlink">
    <xsl:call-template name="to-iso19115-3.mgb-keyword">
  ...
  <!-- Convert a concept to an iso19115-3.mgb keywords.
    If no keyword is provided, only thesaurus section is adaded.
    -->
  <xsl:template name="to-iso19115-3.mgb-keyword">
    ...
    <xsl:apply-templates mode="to-iso19115-3.mgb-keyword" select="." >
  ...
  <xsl:template mode="to-iso19115-3.mgb-keyword"
    ...
    <mri:descriptiveKeywords>
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:call-template name="to-iso19115-3.mgb-md-keywords">
            <xsl:with-param name="withAnchor" select="$withAnchor"/>
            <xsl:with-param name="withThesaurusAnchor" select="$withThesaurusAnchor"/>
            <xsl:with-param name="listOfLanguage" select="$listOfLanguage"/>
            <xsl:with-param name="textgroupOnly" select="$textgroupOnly"/>
  ...
  <xsl:template mode="to-iso19115-3.mgb-keyword" match="*[/root/request/skipdescriptivekeywords]">
    <xsl:param name="textgroupOnly"/>
    <xsl:param name="listOfLanguage"/>
    <xsl:param name="withAnchor"/>
    <xsl:param name="withThesaurusAnchor"/>
  ...
  <xsl:template name="to-iso19115-3.mgb-md-keywords">
    <xsl:param name="textgroupOnly"/>
    <xsl:param name="listOfLanguage"/>
    <xsl:param name="withAnchor"/>
    <xsl:param name="withThesaurusAnchor"/>
    <mri:MD_Keywords>
      ...
      <xsl:copy-of select="geonet:add-iso19115-3.mgb-thesaurus-info($currentThesaurus, $withThesaurusAnchor, /root/gui/thesaurus/thesauri, not(/root/request/keywordOnly))" />
  ...
  <xsl:function name="geonet:add-iso19115-3.mgb-thesaurus-info">
  ...
  <!-- Convert a concept to an iso19115-3.mgb extent -->
  <xsl:template name="to-iso19115-3.mgb-extent">

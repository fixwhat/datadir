...
<xsl:stylesheet version="2.0"
  ...
  -->
  <xsl:template mode="mode-iso19115-3.mgb" match="*[cit:CI_Date]" priority="2000">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>

    <xsl:apply-templates mode="mode-iso19115-3.mgb" select="*/cit:*">
  ...
  <xsl:template mode="mode-iso19115-3.mgb" match="cit:dateType" priority="2000"/>

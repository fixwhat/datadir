...
<xsl:stylesheet version="2.0" xmlns:xsl ="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:include href="metadata-subtemplates.xsl"/>
	...
	<!-- main template - the way into processing iso19115-3.mgb -->
	<xsl:template name="metadata-iso19115-3.mgb">
		<xsl:param name="schema"/>
		<xsl:param name="edit" select="false()"/>
		<xsl:param name="embedded"/>

		<xsl:apply-templates mode="iso19115-3.mgb" select="." >
	...
	<xsl:template mode="iso19115-3.mgb" match="*|@*">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<xsl:template mode="iso19115-3.mgb"
	...
	-->
	<xsl:template mode="iso19115-3.mgb" priority="199" match="*[@gco:nilReason='missing' and geonet:element and count(*)=1]"/>

	<xsl:template mode="iso19115-3.mgb" priority="199" match="*[geonet:element and count(*)=1 and text()='']"/>
	...
	<xsl:template mode="iso19115-3.mgb" match="*[gco:Date|gco:DateTime|gco:Integer|gco:Decimal|gco:Boolean|gco:Real|gco:Measure|gco:Length|gco:Distance|gco:Angle|gco:Scale|gco:RecordType|gcx:MimeFileType]">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<xsl:template mode="iso19115-3.mgb" match="gco:ScopedName|gco:LocalName">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<!-- GML time interval -->
	<xsl:template mode="iso19115-3.mgb" match="gml:timeInterval">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<xsl:template mode="iso19115-3.mgb" match="gml:identifier|gml:axisDirection|gml:descriptionReference">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<xsl:template mode="iso19115-3.mgb" match="*[*/@codeList]">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>

		<xsl:call-template name="iso19115-3.mgbCodelist">
			<xsl:with-param name="schema" select="$schema"/>
			<xsl:with-param name="edit"   select="$edit"/>
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template name="iso19115-3.mgbCodelist">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<xsl:template mode="iso19115-3.mgbGetAttributeText" match="@*">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:variable name="codelistProfile">
					<xsl:choose>
						<xsl:when test="starts-with($schema,'iso19115-3.mgb.')">
							<xsl:copy-of
				...
				<xsl:variable name="codelistCore">
					<xsl:choose>
						...
						<xsl:otherwise>
							<xsl:copy-of
								select="/root/gui/schemas/*[name(.)='iso19115-3.mgb']/codelists/codelist[@name = $qname]/*" />
						</xsl:otherwise>
					</xsl:choose>
				</xsl:variable>
	...
	<xsl:template mode="iso19115-3.mgb" match="mdb:metadataIdentifier[position() = 1]" priority="2">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<xsl:template mode="iso19115-3.mgb" match="cit:date[gco:DateTime|gco:Date]|cit:editionDate" priority="2">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<xsl:template mode="iso19115-3.mgb" match="gml:*[gml:beginPosition|gml:endPosition]|gml:TimeInstant[gml:timePosition]" priority="2">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
		<xsl:for-each select="*">
			<xsl:choose>
	...
	<xsl:template mode="iso19115-3.mgb" match="*[geonet:info/isTemplate='s']" priority="3">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<xsl:template mode="iso19115-3.mgb" match="@gco:isoType"/>

	<!-- ==================================================================== -->
	<!-- Metadata -->
	<!-- ==================================================================== -->

	<xsl:template mode="iso19115-3.mgb" match="mdb:MD_Metadata|*[@gco:isoType='gmd:MD_Metadata']">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
		<xsl:param name="embedded"/>
			...
			<td valign="middle" colspan="2">
				...
				<xsl:if test="/root/gui/config/editor-metadata-relation">
					<div style="float:right;">
						<xsl:call-template name="relatedResourcesiso19115-3.mgb">
							<xsl:with-param name="edit" select="$edit"/>
						</xsl:call-template>
					</div>
				</xsl:if>
		...
		<xsl:choose>
			...
			<xsl:when test="$currTab='metadata'">
				<xsl:call-template name="iso19115-3.mgbMetadata">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="edit"   select="$edit"/>
				</xsl:call-template>
			</xsl:when>
			...
			<xsl:when test="$currTab='ISOMinimum'">
				<xsl:call-template name="iso19115-3.mgbtabs">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="edit"   select="$edit"/>
			...
			<xsl:when test="$currTab='ISOAll'">
				<xsl:call-template name="iso19115-3.mgbComplete">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="edit"   select="$edit"/>
				</xsl:call-template>
			</xsl:when>
			...
			<!-- default -->
			<xsl:otherwise>
				<xsl:call-template name="iso19115-3.mgbSimple">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="edit"   select="$edit"/>
				</xsl:call-template>
			</xsl:otherwise>
	...
	<xsl:template name="iso19115-3.mgbtabs">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<xsl:template name="iso19115-3.mgbComplete">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<xsl:template name="iso19115-3.mgbMetadata">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<xsl:template name="iso19115-3.mgbSimple">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
		...
		<xsl:call-template name="complexElementGui">
			<xsl:with-param name="title" select="/root/gui/strings/metadata"/>
			<xsl:with-param name="content">
				<xsl:call-template name="iso19115-3.mgbSimple2">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="edit"   select="$edit"/>
				</xsl:call-template>
			</xsl:with-param>
	...
	<xsl:template name="iso19115-3.mgbSimple2">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<!-- ===================================================================== -->
	<!-- === iso19115-3.mgb brief formatting === -->
	<!-- ===================================================================== -->

	<xsl:template name="iso19115-3.mgbBrief">
		<metadata>
			<xsl:choose>
				<xsl:when test="geonet:info/isTemplate='s'">
					<xsl:call-template name="iso19115-3.mgb-subtemplate"/>
					<xsl:copy-of select="geonet:info" copy-namespaces="no"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:call-template name="iso19115-3.mgb-brief"/>
				</xsl:otherwise>
			</xsl:choose>
		</metadata>
	</xsl:template>
	...
	<xsl:template name="iso19115-3.mgb-brief">
		...
		<xsl:variable name="langId">
			<xsl:call-template name="getLangIdiso19115-3.mgb">
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:for-each select="cit:name">
							<xsl:call-template name="localisediso19115-3.mgb">
								<xsl:with-param name="langId" select="$langId"/>
							</xsl:call-template>
						</xsl:for-each>
					</xsl:otherwise>
	...
	<xsl:template mode="briefster" match="mri:MD_DataIdentification">
		...
		<xsl:if test="mri:citation/*/cit:title">
			<title>
				<xsl:apply-templates mode="localisediso19115-3.mgb" select="mri:citation/*/cit:title">
		...
		<xsl:if test="mri:abstract">
			<abstract>
				<xsl:apply-templates mode="localisediso19115-3.mgb" select="mri:abstract">
					<xsl:with-param name="langId" select="$langId"></xsl:with-param>
				</xsl:apply-templates>
			</abstract>
		</xsl:if>
		...
		<xsl:for-each select=".//mri:keyword[not(@gco:nilReason)]">
			<keyword>
				<xsl:apply-templates mode="localisediso19115-3.mgb" select=".">
					<xsl:with-param name="langId" select="$langId"></xsl:with-param>
				</xsl:apply-templates>
			</keyword>
		</xsl:for-each>
	...
	<!-- ============================================================================= -->
	<!-- iso19115-3.mgb complete tab template	-->
	<!-- ============================================================================= -->

	<xsl:template name="iso19115-3.mgbCompleteTab">
		<xsl:param name="tabLink"/>
		<xsl:param name="schema"/>
		...
		<xsl:if test="/root/gui/config/metadata-tab/advanced">
			...
			<xsl:call-template name="displayTab">
				...
				<xsl:with-param name="text"    select="/root/gui/schemas/iso19115-3.mgb/strings/resourceLineageTab"/>
				<xsl:with-param name="indent"  select="'&#xA0;&#xA0;&#xA0;'"/>
				<xsl:with-param name="tabLink" select="$tabLink"/>
			</xsl:call-template>
	...
	<xsl:template mode="iso19115-3.mgbIsEmpty" match="*|@*|text()">
		<xsl:choose>
			<!-- normal element -->
			<xsl:when test="*">
				<xsl:apply-templates mode="iso19115-3.mgbIsEmpty"/>
			</xsl:when>
			<!-- text element -->
...
mdb:MD_Metadata/mdb:defaultLocale/lan:PT_Locale/lan:language/lan:LanguageCode 
	...
	-->
	<xsl:template mode="iso19115-3.mgb" match="mdb:defaultLocale|geonet:child[string(@name)='defaultLocale']" priority="1">
		<xsl:param name="schema" />
		<xsl:param name="edit" />
		<xsl:choose>
			<xsl:when test="$edit = true()">
	...
	<xsl:template mode="iso19115-3.mgb"
		...
		<xsl:call-template name="localizedCharStringField_iso19115-3.mgb">
			<xsl:with-param name="schema" select="$schema" />
			<xsl:with-param name="edit" select="$edit" />
	...
	-->
	<xsl:template name="localizedCharStringField_iso19115-3.mgb" >
		<xsl:param name="schema" />
		<xsl:param name="edit" />
		...
		<xsl:variable name="widget">
			<xsl:if test="$edit=true()">
				<xsl:variable name="tmpFreeText">
					<xsl:call-template name="PT_FreeText_Tree_iso19115-3.mgb" />
				</xsl:variable>
				...
				<xsl:variable name="mainLangId">
					<xsl:call-template name="getLangIdFromMetadataiso19115-3.mgb">
	...
	<!--
		Create a PT_FreeText_Tree_iso19115-3.mgb for multilingual editing.
	...
	-->
	<xsl:template name="PT_FreeText_Tree_iso19115-3.mgb">
		<xsl:variable name="mainLang"
		...
		<xsl:for-each select="$languages">
			...
			<xsl:variable name="code">
				<xsl:call-template name="getLangCodeiso19115-3.mgb">
					<xsl:with-param name="md"
						select="ancestor-or-self::*[name(.)='mdb:MD_Metadata' or contains(@gco:isoType,'MD_Metadata')]" />
	...
	<xsl:template name="iso19115-3.mgb-javascript"/>

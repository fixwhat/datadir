<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="iso19115-3.mgb-subtemplate">
    
    <xsl:variable name="subTemplateTitle">
      <xsl:apply-templates mode="iso19115-3.mgb-subtemplate" select="."/>
    </xsl:variable>
  ...
  <!-- Subtemplate mode -->
  <xsl:template mode="iso19115-3.mgb-subtemplate" match="cit:CI_Responsibility">
  ...
  <xsl:template mode="iso19115-3.mgb-subtemplate" match="gex:EX_Extent">
    <!-- TODO : multilingual subtemplate are not supported. There is
  ...
  <xsl:template mode="iso19115-3.mgb-subtemplate" match="mri:MD_Keywords">
    <xsl:variable name="langId">
      <xsl:call-template name="getLangId">
        <xsl:with-param name="langGui" select="/root/gui/language"/>
        <xsl:with-param name="md" select="."/>
  ...
  <xsl:template mode="iso19115-3.mgb-subtemplate" match="mrd:MD_Distribution">
    <xsl:value-of
      select="string-join(mrd:transferOptions/mrd:MD_DigitalTransferOptions/mrd:onLine/cit:CI_OnlineResource/cit:linkage/*, ' ,')"
    />
  </xsl:template>
  
  <xsl:template mode="iso19115-3.mgb-subtemplate" match="mco:MD_LegalConstraints">
    <xsl:value-of
  ...
  <xsl:template mode="iso19115-3.mgb-subtemplate" match="*"/>

...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    -->
    <xsl:template name="localisediso19115-3.mgb" mode="localisediso19115-3.mgb" match="*[gco:CharacterString or lan:PT_FreeText]">
    ...
    <xsl:template mode="localisediso19115-3.mgb" match="*[not(gco:CharacterString or lan:PT_FreeText)]">
        <xsl:param name="langId"/>
			<xsl:value-of select="*[1]"/>
	</xsl:template>

	<!-- Check if the element has hidden subelements -->
    <xsl:template mode="localisediso19115-3.mgb" match="*[@gco:nilReason='withheld' and count(./*) = 0 and count(./@*) = 1]" priority="100">
        <xsl:value-of select="/root/gui/strings/concealed"/>
	</xsl:template>

    <!-- Map GUI language to iso3code -->
    <xsl:template name="getLangIdiso19115-3.mgb">
    ...
    <!-- Get lang #id in metadata PT_Locale section,  deprecated: if not return the 2 first letters
        ...
        -->
    <xsl:template name="getLangIdFromMetadataiso19115-3.mgb">
        <xsl:param name="md"/>
        <xsl:param name="lang"/>
    ...
    <xsl:template name="getLangCodeiso19115-3.mgb">
        <xsl:param name="md"/>
        <xsl:param name="langId"/>
    ...
    <xsl:template name="getMetadataTitleiso19115-3.mgb">
        <xsl:param name="uuid"/>
	...
	<!-- Display related metadata records. Related resource are only iso19115-3.mgb/iso19139/119 or iso19110 metadate records for now.
	...
	-->
	<xsl:template name="relatedResourcesiso19115-3.mgb">
		<xsl:param name="edit"/>

		<xsl:variable name="metadata" select="/root/mds:MD_Metadata|/root/*[contains(@gco:isoType,'MD_Metadata')]"/>
		<xsl:if test="starts-with(geonet:info/schema, 'iso19115-3.mgb') or geonet:info/schema = 'iso19110'">
			...
			<xsl:variable name="capabilitiesUrl">
				<xsl:call-template name="getServiceURLiso19115-3.mgb">
		        					...
		        					<ul>
		        						<li>
			        						<a class="arrow" href="metadata.show?uuid={uuid}">
				        						<xsl:call-template name="getMetadataTitleiso19115-3.mgb">
		        	...
		        	<xsl:if test="(normalize-space($parent)!='' or $children or $edit) and geonet:info/schema != 'iso19110'">
		        		...
		        		<xsl:if test="normalize-space($parent)!='' or $children">
		        			<ul>
		        				...
		        				<li>
		        					<ul>
		        						<li><xsl:call-template name="getMetadataTitleiso19115-3.mgb">
		        								<xsl:with-param name="uuid" select="$uuid"/>
  ...
  <xsl:template mode="iso19115-3.mgbIsEmpty" match="*|@*|text()">
	...
	<xsl:template name="getServiceURLiso19115-3.mgb">
